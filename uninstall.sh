...
# 确认卸载
log_warning "即将卸载每日业务总结，这将删除所有相关文件"
...
if [ -f "$SKILL_DIR/config.json" ]; then
    ...
    mkdir -p "$BACKUP_DIR"
    BACKUP_FILE="$BACKUP_DIR/daily-summary_config_$(date +%Y%m%d_%H%M%S).json"
    cp "$SKILL_DIR/config.json" "$BACKUP_FILE"
    log_success "配置文件已备份到: $BACKUP_FILE"
fi

# 询问是否删除定时任务
if crontab -l 2>/dev/null | grep -q "daily_summary.py"; then
    read -p "是否删除定时任务？(y/n) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        crontab -l 2>/dev/null | grep -v "daily_summary.py" | crontab -
        log_success "定时任务已删除 ✓"
    fi
fi
...
echo ""
echo "💡 如需重新安装，请运行:"
echo "   git clone https://github.com/fengzitong/daily-summary.git"
echo "   cd daily-summary"
